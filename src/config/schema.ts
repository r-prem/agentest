...
export const thresholdsSchema = z.record(z.number().min(0).max(5)).refine(
  ...
  (rec) => ({
    message: `Unknown metric name(s) in thresholds: ${Object.keys(rec).filter((k) => !(VALID_METRIC_NAMES as readonly string[]).includes(k)).join(', ')}. Valid names: ${VALID_METRIC_NAMES.join(', ')}`,
  }),
)
...
export const configSchema = z.object({
  ...
  provider: z.enum(['openai', 'anthropic', 'google', 'ollama', 'openai-compatible']).default('anthropic'),
  ...
  reporters: z
    .array(z.enum(['console', 'json', 'github-actions']))
    .default(['console']),
